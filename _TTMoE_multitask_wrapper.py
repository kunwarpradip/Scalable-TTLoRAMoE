...
class MoEsparseRouting(nn.Module):
    ...
    def count_labels_of_experts_selection(self, gates):
        ...
        for row in gates:
            ...
            if idx == 0:
                counts[expert_key[0]] += 1
            elif idx == 1:
                counts[expert_key[1]] += 1
            elif idx == 2:
                counts[expert_key[2]] += 1
            elif idx == 3:
                counts[expert_key[3]] += 1

        counts["total"] = sum(counts.values())
        counts[expert_key[0]] = (counts[expert_key[0]]/counts["total"])*100
        counts[expert_key[1]] = (counts[expert_key[1]]/counts["total"])*100
        counts[expert_key[2]] = (counts[expert_key[2]]/counts["total"])*100
        counts[expert_key[3]] = (counts[expert_key[3]]/counts["total"])*100
        return counts
    ...
    def forward(self, X, *args, **kwargs):
        ...
        if "roberta" in self.model_name:
            ...
            print(f"\Selected Experts of this Batch size {B}\n", 
            ...
            for layer in self.base_module.layer:
                ...
                list_query_cores = [[] for _ in range(8)]
                ...
                ##################################################For Value######################################
                # (a) Collect query TT-cores for all experts
                list_value_cores = [[] for _ in range(8)]
            ...
            print("Type of X in forward of MoESparse", X.dtype)
            print("shape of X before embed_tokens",X.shape)
            X = self.base_module.embed_tokens(X).to(self.device)
            print("shape of X after embed_tokens",X.shape)
            B = X.size(0)
            pooled_hidden_states = X.float().mean(dim=1)
            print("shape of pooled_hidden_states",pooled_hidden_states.shape)
            logits = self.router(pooled_hidden_states)
            print("shape of logits",logits.shape)
            self.gates = F.gumbel_softmax(logits, tau=gumbel_temperature, hard=True).to(self.device)
            print("shape of gates",self.gates.shape)
            print(f"\Selected Experts of this Batch size {B}\n", 
                self.count_labels_of_experts_selection(self.gates))
            layer_idx = 0
            ...
            # Forward through transformer layers
            X = X_temp
            print("type of X before passing to base_module", X.dtype)
            model_output = self.base_module(X, *args, **kwargs)
...
class MoEsparseRoutingForClassification(nn.Module):
    ...
    def custom_classifier_out_proj_forward(self, X,
        ...
        print("shape of selected_weights", selected_weights.shape)
        if "llama"  in self.model_name:
            ...
            X= X.unsqueeze(1)
            out = (X * selected_weights).sum(2) + selected_biases
        print("output shape from custom out_proj_forward", out.shape)
        return out
    ...
    def forward(self, X, *args, **kwargs):
        ...
        if "roberta" in self.model_name:
            ...
            gates_dense = gates.view(B, 4, 1, 1).to(self.device)  # Shape: [B, 4, 1, 1] for dense weights
            gates_out_proj = gates.view(B, 4, 1).to(self.device)  # Shape: [B, 4, 1] for out_proj biases
            ...
            self.base_module.dense.forward = partial(self.custom_classifier_dense_forward,
                                                    selected_weights=selected_dense_weights, 
                                                    selected_biases=selected_dense_biases)

            self.base_module.out_proj.forward = partial(self.custom_classifier_out_proj_forward, 
            ...
            self.base_module.forward = partial(self.custom_classifier_out_proj_forward, 
