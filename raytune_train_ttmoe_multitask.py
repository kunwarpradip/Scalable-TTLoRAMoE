...
from utils import get_tokenizer, load_local_dataset, load_mixed_datasets, parse_experts, get_ttlora_shape, get_ttlora_rank
...
def train_moe_without_ray(config):
    ...
    if "roberta" in config["model_name"]:
        ...
        model.classifier = MoEsparseRoutingForClassification(model.classifier, 
                                config,
                                router=model.roberta.encoder,)
        
    elif "llama" in config["model_name"]:
        model.model = MoEsparseRouting(model.model, config=config)
        model.score = MoEsparseRoutingForClassification(model.score, 
                                config,
                                router=model.model,)
    else:
    ...
    if config["dataload_type"] == "single":
        ...
        train_dataset_dict, val_dataset_dict = load_mixed_datasets(config["multiple_datasets"], config["tokenizer_path"])
...
def main():
    ...
    model_name = "llama-3-8b" # options: roberta-base, llama-3.2-3b, llama-3-8b, llama-3-70b
    ...
    config = {
        ...
        "model_path" : '/lustre/vescratch1/ceodspspectrum/llms/llama31-8b/checkpoints/',
        "tokenizer_path" : '/lustre/vescratch1/ceodspspectrum/llms/llama31-8b/checkpoints/',
        "device": device,  
        ...
        #hyperparameters
        "router_type" : tune.choice(["single_layer",  "multi_layer", "attention"]),
        "gumbel_temperature": tune.choice([0.4,0.6,0.8,1,1.2,1.4,1.6]),
        "learning_rate": tune.choice([1e-3,1e-4,1e-5,5e-5,5e-4])
    }
    scheduler = HyperBandScheduler(
                    time_attr="training_iteration",
                    max_t=100,  # Maximum number of iterations for each configuration
    ...
    reporter = CLIReporter(
        parameter_columns = [ "router_type", "gumbel_temperature", "learning_rate"],
        metric_columns = ["val_acc", "trainable_params"]
    )

    analysis = tune.run(
        train_moe_without_ray ,
        resources_per_trial={"cpu": 4, "gpu": args.gpus},
        config=config,
        num_samples=400,  #how many different combinations to try
        scheduler=scheduler,
        ...
        metric = "val_acc",
        mode="max",
        storage_path = "/lustre/vescratch1/ceodspspectrum/pradip_moe/",
        name="tune_llama31_8b"+'_'+args.data,
        fail_fast = "raise"
    )
    ...
    #filename= f"{args.data}_ray_tune_results_llama31_8b.cvs"
